...
## Asset extension policy

Published artifacts may include static files alongside generated HTML. Extensions fall into three classes:

| Class | Policy | Examples |
|---|---|---|
| **Whitelisted** | Always allowed when publish is otherwise permitted | `.json`, `.jsonp`, `.yaml`, `.yml`, `.txt`, `.xml`, `.toml`, `.csv`, `.ndjson`, images (`.png`, `.jpg`, …), `.svg`, `.pdf`, generated `.html` |
| **Unlisted** | Blocked unless the matching auto-approval rule has `allow_unlisted_assets = true` | `.zip`, `.woff2`, `.exe`, any extension not on the whitelist |
| **Forbidden** | Never allowed in repo uploads | `.js`, `.mjs`, `.cjs` (except platform runtime under `_nrdocs/`) |

Repo-uploaded `.html` is not collected by the CLI; only nrdocs-generated HTML is expected in artifacts.

At each publish, the Worker evaluates `matchRules(fullName)` and passes `allow_unlisted_assets` from the matched rule into archive validation. The CLI calls `GET /api/publish-capabilities` with OIDC before render so CI fails early with the same policy.

Operators grant unlisted consent per rule:

```bash
nrdocs rules add 'OWNER/*' --access password --allow-unlisted-files true
...
Minimum supported types include:

```text
.html    text/html; charset=utf-8
.css     text/css; charset=utf-8
.json    application/json; charset=utf-8
.jsonp   text/plain; charset=utf-8   (never application/javascript)
...
.pdf     application/pdf
.zip     application/zip   (only when allow_unlisted_assets permitted the upload)
```

Unlisted extensions that were permitted by rule may be served with a known MIME map entry or conservative `application/octet-stream` where no mapping exists.
