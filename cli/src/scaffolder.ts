          ...
          if [ "$exchange_code" -ge 200 ] && [ "$exchange_code" -lt 300 ]; then
            ...
            case "$err" in
              *"not approved"*|*"Repo is not approved"*)
                echo "::notice::Registered with the Control Plane. Publishing is skipped until an operator approves (this job exits without polling — no long Actions wait)."
                if [ -n "\${GITHUB_STEP_SUMMARY:-}" ]; then
                  {
                    echo "## Awaiting operator approval"
                    echo "After an operator approves this repo, **re-run this workflow** (Actions → run workflow) or **push** the publish branch again to publish to Cloudflare."
                  } >> "\$GITHUB_STEP_SUMMARY"
                fi
                exit 0
                ;;
