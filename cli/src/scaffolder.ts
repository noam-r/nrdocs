    ...
    steps:
      ...
      - name: Build payload and publish
        ...
        run: |
          ...
          # One-shot OIDC publish-credentials. Operator approval is asynchronous — CI must not poll/sleep (Actions billing).
          oidc_json=$(curl -sSf -H "Authorization: Bearer \${ACTIONS_ID_TOKEN_REQUEST_TOKEN}" "\${ACTIONS_ID_TOKEN_REQUEST_URL}&audience=\${audience}")
          oidc_token=$(echo "$oidc_json" | jq -r '.value')
          if [ -z "$oidc_token" ] || [ "$oidc_token" = "null" ]; then
            echo "::error::Failed to acquire GitHub OIDC token for publish-credentials"
            exit 1
          fi
          ...
          elif [ "$exchange_code" = "409" ]; then
            err=$(jq -r '.error // empty' exchange.json 2>/dev/null || true)
            case "$err" in
              *"not approved"*|*"Repo is not approved"*)
                echo "::warning::nrdocs: DOCUMENTATION WAS NOT PUBLISHED — Control Plane refused publish credentials (repo not operator-approved yet)."
                echo "::warning::nrdocs: Operators: run nrdocs admin approve <repo-id> (see registration JSON / nrdocs admin list --all)."
                echo "::warning::nrdocs: Repo owners: approval does not re-run this job. After approval, push the publish branch again or use Actions → Run workflow."
                echo "nrdocs: This job exits successfully so Actions does not retry in a loop; reader URLs are unchanged until a later run publishes."
                if [ -n "\${GITHUB_STEP_SUMMARY:-}" ]; then
                  {
                    echo "## Documentation was not published"
                    echo ""
                    echo "**Why:** POST /oidc/publish-credentials returned HTTP **409**. The Control Plane has not approved this GitHub repo yet, so no publish token was issued."
                    echo ""
                    echo "### Operators"
                    echo "Run: nrdocs admin approve <repo-id> (repo id from registration output or nrdocs admin list --all)."
                    echo ""
                    echo "### Repository owners (after approval)"
                    echo "Approval does not publish by itself. Start a **new** workflow run, for example:"
                    echo "- GitHub: Actions → this workflow → Run workflow → branch **\${GITHUB_REF_NAME}**"
                    echo "- Or: git commit --allow-empty -m \\\"chore: trigger publish\\\" && git push origin \${GITHUB_REF_NAME}"
                    echo ""
                    echo "**Until then, reader URLs will not update.**"
                  } >> "\$GITHUB_STEP_SUMMARY"
                fi
                exit 0
                ;;
          ...
          # Binary assets under content/ (extensions must match Control Plane allowlist).
          assets_file=\$(mktemp)
          echo '{}' > "\$assets_file"
          while IFS= read -r -d '' file; do
            relative="\${file#"$DOCS_DIR/content/"}"
            rel_lc=\$(printf '%s' "\$relative" | tr '[:upper:]' '[:lower:]')
            case "\$rel_lc" in
            ...
            b64=\$(base64 -w0 < "\$file")
            tmp_assets=\$(mktemp)
            jq --arg k "\$relative" --arg v "\$b64" '. + {($k): $v}' "\$assets_file" > "\$tmp_assets"
            mv "\$tmp_assets" "\$assets_file"
          done < <(find "\$DOCS_DIR/content" -type f ! -name '*.md' -print0 | sort -z)
          ...
          if [ -f "$DOCS_DIR/allowed-list.yml" ]; then
            jq -n \
              ...
              '{repo_content: {project_yml: $project_yml, nav_yml: $nav_yml, allowed_list_yml: $allowed_list_yml, pages: $pages[0], assets: $assets[0]}}' > "$payload_file"
          else
            jq -n \
              --rawfile project_yml "$DOCS_DIR/project.yml" \
              --rawfile nav_yml "$DOCS_DIR/nav.yml" \
              --slurpfile pages "$pages_file" \
              --slurpfile assets "\$assets_file" \
              '{repo_content: {project_yml: $project_yml, nav_yml: $nav_yml, allowed_list_yml: null, pages: $pages[0], assets: $assets[0]}}' > "$payload_file"
          fi
